...
app.get('/api/stats', async (req, res) => {
  try {
    ...
    if (personasResult.error) {
      console.error('Error obteniendo conteo de personas:', personasResult.error);
    }
    if (delincuentesResult.error) {
      console.error('Error obteniendo conteo de delincuentes:', delincuentesResult.error);
    }
...
app.use((error, req, res, next) => {
  ...
  if (error.code && error.code.startsWith('23')) { // PostgreSQL errors
